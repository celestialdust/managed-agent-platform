...
def _a_grant_set_iteration_does_not_already_sort() -> frozenset[str]:
    """Five tool names whose `frozenset` iteration order is not sorted order.

    Found at run time rather than written down, because set iteration depends on
    PYTHONHASHSEED and any fixed five names are a per-run draw. The five that used to
    be written here landed in sorted order on 18 of 3000 seeds -- 0.60% -- and on the
    run that drew one of them, a commit touching nothing near this code went red.

    A caller wanting more names would only make the draw rarer, which is worse: a
    once-a-year flake costs more to diagnose than a once-a-month one. Searching removes
    the draw instead. It is deterministic for a given seed -- a fixed pool, a fixed
    combination order, first hit wins -- so two runs under one seed pick the same five,
    and every seed gets a fixture that satisfies the property.

    Raises rather than falling back to the first combination, because a fixture that
    quietly came back sorted would leave the caller's assertion unable to tell a sorted
    write from an unsorted one, which is the exact false negative this exists to remove.
    """
    for names in itertools.combinations(_A_NAME_POOL, 5):
        candidate = frozenset(names)
        if list(candidate) != sorted(candidate):
            return candidate
    raise AssertionError(
        "no five names in the pool iterate out of sorted order under this "
        "PYTHONHASHSEED, which should not happen -- widen _A_NAME_POOL"
    )
...
async def test_the_stored_grant_is_sorted_rather_than_in_set_order(
    ...
    registry = PostgresSessionRegistry(engine)
    tenant_id = TenantId(uuid.uuid4())
    grant = _a_grant_set_iteration_does_not_already_sort()
    assert list(grant) != sorted(grant), (
        "the fixture came back in sorted order, so list(grant) and sorted(grant) are "
        "the same sequence and the assertion below cannot tell them apart -- it would "
        "pass while `create` did no sorting at all. The search above is supposed to "
        "make this unreachable; reaching it means that function is broken, not that "
        "the draw went badly."
    )
